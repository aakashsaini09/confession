...
const Card = ({ data }: { data: Confession }) => {
  ...
  // const [likeDisLike, setlikedisLike] = useState(Boolean)
  const likeDislikeFunction = async (id: string, operation: boolean) => {
    

   

    const res = await axios.put(`/api/like-dislike/${id}`, { operation: operation });
      ...
      <a href="#" className="flex flex-col bg-white border border-gray-900 rounded-lg shadow-sm md:flex-row sm:max-w-lg md:max-w-xl sm:h-36 md:h-52 hover:bg-gray-100">
        <div className="flex flex-col justify-between p-4 leading-normal">
          ...
          <div className='w-full flex gap-5'>
            ...
            : 
            <div className='flex gap-1' onClick={() => likeDislikeFunction(data._id, true)}>
            ...
            <div className='flex gap-1'>
              ...
              <div className='flex gap-1' onClick={() => likeDislikeFunction(data._id, false)}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
